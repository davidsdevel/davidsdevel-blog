...
class DB {
  ...
  async init() {
    try {
      let createPostPromise;
      let createUsersPromise;
      let createViewsPromise;
      ...
      if (!existsViews) {
        createViewsPromise = this.db.schema.createTable('views', (table) => {
          ...
          if (this.client === 'mysql') { table.dateTime('time'); } else { table.timestamp('time'); }
        });
      }
  ...
  async savePost(data) {
    try {
      const {
        title, description, tags, content, image, url, category,
      } = data;
  ...
  async addFCMToken(token, email) {
    try {
      if (email !== undefined && email !== 'undefined') { await this.db('users').update({ fcmToken: token }).where({ email }); } else { await this.db('users').insert({ fcmToken: token }); }
      ...
      const promises = data.feed.entry.map(async (e) => {
        ...
        if (Array.isArray(e.category)) {
          tags = e.category.map((cat) => {
            if (/https?:\/\/schemas\.google\.com\//.test(cat._attributes.scheme)) { return undefined; }
  ...
  async getStats() {
    try {
      ...
      views.forEach(({
        time, url, referer, os, browser, country,
      }) => {
