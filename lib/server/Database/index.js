...
class DB extends Database {
    ...
    try {
      ...
      if (!existsViews) {
        promisesArray.push(this.db.schema.createTable('views', (table) => {
          table.string('url').references('posts.url');
  ...
  connect(client, user, password, server, port, database) {
    ...
    if (client === 'pg') { protocol = `postgres://${user}:${password}@${server}:${port}/${database}`; } else if (client === 'mysql') { protocol = `mysql://${user}:${password}@${server}:${port}/?db=${database}&charset=utf8&connectTimeout=500`; } else {
      protocol = {
        filename: './data.db',
      };
    }
  ...
  async testConnection(client, user, password, server, port, database) {
    try {
      ...
      if (client === 'pg') { protocol = `postgres://${user}:${password}@${server}:${port}/${database}`; } else if (client === 'mysql') { protocol = `mysql://${user}:${password}@${server}:${port}/${database}?charset=utf8&connectTimeout=500`; } else {
        protocol = {
          filename: './data.db',
        };
      }
