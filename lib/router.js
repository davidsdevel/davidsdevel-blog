...
const DB = process.env.NODE_ENV !== "production" ? require("../lib/TestDB") : require("../lib/DB");
...
class Router {
	static async index({req, res}, app) {
		try {
			var queryData;
			const {page} = req.query;
			const posts = await db.getPublishPosts();

			if (page) {
				const pageNumber = parseInt(page) * 10;
				queryData = posts.slice(pageNumber - 10, pageNumber);
			} else {
				queryData = posts.slice(0, 10);
			}
			if (!app) res.json(queryData);
		} catch(err) {
			console.error(err);
	...
	static async post({req, res, next}, app) {
		...
		try {
			...
			var request;
			if (!app) request = await db.getPostByURL(url);
			else request = await db.getPostByURL(`${category}/${title}`);
