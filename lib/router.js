...
class Router {
	...
	async sitemap({req, res}) {
		try {
			...
			const mapped = posts.map(({category, url, updated}) => `<url><changefreq>monthly</changefreq><loc>https://blog.davidsdevel.com/${category}/${url}</loc><lastmod>${updated}</lastmod><priority>1</priority></url>`)
	...
	async feed({req, res}) {
		try {
			...
			posts.forEach(({title, url, description, content, updated, image, category}) => {
				...
				feed.addItem({
					title,
					id: `https://davidsdevel.blogspot.com/${category}/${url}`,
					link: `https://davidsdevel.blogspot.com/${category}/${url}`,
					description,
					content,
					author: [
						{
