...
export default class Dashboard extends Component {
  ...
  async logout() {
    try {
      ...
      if (req.status >= 400) { alert('Error al cerrar la sesión'); } else {
  ...
  render() {
    ...
    const load = (
      <div id="load-container" style={{ display, opacity }}>
        <span>Cargando</span>
        <div id="load-points">
          <span className="one" />
          <span className="two" />
        ...
        <aside style={{display: showMenu ? 'inline-block' : 'none'}}>
          <ul>
			...
			*/}
            ...
            <li onClick={() => this.changeTab('colaborators')}>
              <img src="/images/colaborators.png" />
            </li>
